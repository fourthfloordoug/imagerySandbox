...
timeSeriesTestInput <- testDataTable %>% filter(trial == 22) %>% select(-trial)

#label the data with a time block index.

timeBlocks <- seq(from=0,to=100,by=10)

timeSeriesTestInput %<>% mutate(timeBlock = cut(time,breaks = timeBlocks,labels=FALSE))

timeSeriesTestInputExpanded <- 1:length(timeBlocks) %>% map(.f=function(blockIndex){
  
  timeSeriesTestInput %>% filter(timeBlock <= blockIndex) %>% mutate(trial=blockIndex) %>% bind_rows()
...
numTimeSeriesTrials = 10;
timeSeriesTestdldtData <-
  convertSamplesToDValDt(timeSeriesTestInputExpanded,typeNames,numTimeSeriesTrials)

timeSeriesTestdldtImages <- 
  createImages(timeSeriesTestdldtData,typeNames,numTimeSeriesTrials,limits,bins)

```

```{r}

timeSeriesTestImagesInRows <- widenImageDataFrame(timeSeriesTestdldtImages,numPixels)
timeSeriesTestPredictionTable <-
  testKerasModelForProbType(timeSeriesTestImagesInRows,kerasModel)

```

```{r}

timeSeriesTestPredictionTable %>% ggplot(aes(x=trial,y=probability,color=predictedType)) + geom_line() + geom_point() + facet_wrap(~truth)
...
process3rainingDataTable <- generateSamples(process3GPs,typeNames,maxTimesForBins,numProfilesPerTimeBin,numPointsPerFrofileRanges)
...
```{r}
process2DldtTrainingImages <- createImages(process2DldtData,typeNames,totalProfilesPerType,limits,bins)

process3DldtTrainingImages <- createImages(process3DldtData,typeNames,totalProfilesPerType,limits,bins)
...
type1trial31RGB = dldtTrainingImages %>% filter(type=='truth1',trial==31)  %>%
  mutate(value2 = process2Type1Trail31$value,value3 = process3Type1Trial31$value) %>%
...
type3trial81RGB = dldtTrainingImages %>% filter(type=='truth3',trial==81)  %>%
  mutate(value2 = process2Type3Trail81$value,value3 = process3Type3Trial81$value) %>%
  mutate(rgb.val=rgb(value,value2,value3))
...
rgbTrainingImagesInRows <-
  bind_cols(allTrainingImagesInRows,
            process2TrainingImagesInRows,
            process3TrainingImagesInRows)
...
process2DldtTestImages <- 
  createImages(process2TestdldtData,typeNames,totalProfilesPerType_test,limits,bins)

process3DldtTestImages <- 
  createImages(process3TestdldtData,typeNames,totalProfilesPerType_test,limits,bins)
...
process2TimeSeriesTestInput <- process2TestDataTable %>% filter(trial == 22) %>% select(-trial)

process3TimeSeriesTestInput <- process3TestDataTable %>% filter(trial == 22) %>% select(-trial)
...
process2TimeSeriesTestdldtImages <- 
  createImages(process2TimeSeriesTestdldtData,typeNames,numTimeSeriesTrials,limits,bins)
process3TimeSeriesTestdldtImages <- 
  createImages(process3TimeSeriesTestdldtData,typeNames,numTimeSeriesTrials,limits,bins)
...
process2TimeSeriesTestImagesInRows <-
  widenImageDataFrame(process2TimeSeriesDldtTestImages,numPixels) %>%
  select(-c(trial,type))

process3TimeSeriesTestImagesInRows <-
  widenImageDataFrame(process3TimeSeriesDldtTestImages,numPixels) %>%
  select(-c(trial,type))


rgbTimeSeriesTestImagesInRows <-
  bind_cols(timeSeriesTestImagesInRows,
            process2TimeSeriesTestImagesInRows,
            process3TimeSeriesTestImagesInRows,)
